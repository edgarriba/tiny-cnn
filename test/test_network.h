...
TEST(network, add) {
  ...
  EXPECT_EQ(net.depth(), static_cast<size_t>(1));
}
...
TEST(network, manual_init) {
  ...
  EXPECT_EQ(c1_w->size(), static_cast<size_t>(9));
  EXPECT_EQ(c1_b->size(), static_cast<size_t>(1));
  EXPECT_EQ(f1_w->size(), static_cast<size_t>(2));
